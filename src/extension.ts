...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('apiStressStudioView', provider)
    );
...
class ApiStressViewProvider implements vscode.WebviewViewProvider {
    ...
    private async executeStressTest(params: { url: string; method: string; total: number; concurrency: number; body?: string }) {
            ...
            return new Promise<void>((resolve) => {
                ...
                try {
                    ...
                    if (body && method && method.toUpperCase() !== 'GET') {
                        try { req.write(body); } catch { /* ignore */ }
                    }
                    req.end();
                } catch {
                    failure++;
